...
var { attachLiveSessions } = require("./project-live-sessions");
...
function createProjectContext(opts) {
  ...
  // --- Live CLI session engine (delegated to project-live-sessions.js) ---
  var _liveSessions = attachLiveSessions({
    cwd: cwd,
    sm: sm,
    send: send,
  });
  ...
  function handleMessage(ws, msg) {
    ...
    // --- Live CLI sessions (delegated to project-live-sessions.js) ---
    if (_liveSessions.handleLiveSessionsMessage(ws, msg)) return;
