...
function createServer(opts) {
  ...
  var appHandler = function (req, res) {
    ...
    // All live Claude CLI sessions across every working directory, from
    // Claude Code's process registry. Annotated with the matching Clay
    // project slug when one exists (for click-to-attach in the home hub).
    if (req.method === "GET" && fullUrl === "/api/live-sessions") {
      if (!isRequestAuthed(req)) {
        res.writeHead(401, { "Content-Type": "application/json" });
        res.end(JSON.stringify({ error: "unauthorized" }));
        return;
      }
      var liveRegistry = require("./live-registry");
      var liveEntries = liveRegistry.readLiveRegistry() || [];
      var cwdToSlug = {};
      projects.forEach(function (ctx, slug) {
        if (ctx && ctx.cwd) cwdToSlug[ctx.cwd] = slug;
      });
      var liveOut = liveEntries.map(function (e) {
        return {
          ...
          projectSlug: cwdToSlug[e.cwd] || null,
        };
      });
      liveOut.sort(function (a, b) { return (b.startedAt || 0) - (a.startedAt || 0); });
      res.writeHead(200, { "Content-Type": "application/json" });
      res.end(JSON.stringify({ sessions: liveOut }));
      return;
    }
