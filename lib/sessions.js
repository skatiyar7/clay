...
function createSessionManager(opts) {
  ...
  function switchSession(localId, targetWs, transform) {
    ...
    _send({ type: "session_switched", id: localId, cliSessionId: session.cliSessionId || null, loop: session.loop || null, vendor: session.vendor || null, hasHistory: (session.history && session.history.length > 0), capabilities: _vendorCaps, isProcessing: !!session.isProcessing, liveMirror: !!session.liveMirror });
