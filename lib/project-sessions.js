...
function attachSessions(ctx) {
  ...
  var liveSessions = ctx.liveSessions;
  var getProjectList = ctx.getProjectList;
  var getProjectCount = ctx.getProjectCount;

  // Attach a live-mirror tail when resuming a CLI session that is still
  // actively running in a terminal on the host (recent file activity).
  function maybeAttachLiveMirror(resumed) {
    if (!resumed || !resumed.cliSessionId || !liveSessions) return;
    if ((resumed.vendor || sm.defaultVendor || "claude") !== "claude") return;
    if (resumed.isProcessing) return; // Clay owns the query — never mirror
    if (liveSessions.isLiveCliSession(resumed.cliSessionId)) {
      liveSessions.attachTail(resumed);
    }
  }
  ...
  function handleSessionsMessage(ws, msg) {
    ...
    // CLI sessions are auto-adopted into the unified session list on server
    // start (see sessions.js adoptOrphanedCliSessions). resume_session is
    // still used by the live-sessions UI (sidebar Live terminal section and
    // home hub card) to attach sessions started after boot and begin the
    // live mirror.
    if (msg.type === "resume_session") {
      ...
      Promise.all([
        cliSess.readCliSessionHistory(null, cwd, msg.cliSessionId),
        titlePromise
      ...
      adapter.listSessions({ dir: cwd }).then(function(sdkSessions) {
        var filtered = sdkSessions.filter(function(s) {
          ...
          return {
            ...
            live: liveSessions ? liveSessions.isLiveCliSession(s.sessionId) : false,
          };
        });
        ...
        cliSessions.listCliSessions(cwd).then(function(sessions) {
          ...
          for (var li = 0; li < filtered.length; li++) {
            filtered[li].live = liveSessions ? liveSessions.isLiveCliSession(filtered[li].sessionId) : false;
          }
          sendTo(ws, { type: "cli_session_list", sessions: filtered });
        }).catch(function() {
    ...
    if (msg.type === "switch_session") {
      if (msg.id && sm.sessions.has(msg.id)) {
        ...
        var _swModels = (sm.modelsByVendor && sm.modelsByVendor[_swVendor]) || sm.availableModels || [];
