...
// Attach a CLI session once the project's websocket is connected. Used by the
// home hub's live-sessions card (click-to-attach across project switches).
export function attachCliSessionAfterConnect(cliSessionId) {
  var tries = 0;
  var timer = setInterval(function () {
    var ws = getWs();
    if (ws && ws.readyState === 1 && store.get('connected')) {
      clearInterval(timer);
      ws.send(JSON.stringify({ type: "resume_session", cliSessionId: cliSessionId }));
    } else if (++tries > 40) {
      clearInterval(timer);
    }
  }, 250);
...
export function handleAddProjectResult(msg) {
  ...
  if (msg.ok) {
    ...
    if (msg.slug) {
      if (msg.existing && msg.slug === store.get('currentSlug')) {
        // Already on this project — no switch needed
      } else {
        switchProject(msg.slug);
      }
      // Home hub live-sessions card: attach the session that triggered the add
      var pendingAttach = store.get('hubPendingAttach');
      if (pendingAttach) {
        store.set({ hubPendingAttach: null });
        attachCliSessionAfterConnect(pendingAttach);
      }
    }
  } else {
    store.set({ hubPendingAttach: null });
