...
// --- Session state ---
var cachedSessions = [];
var cachedLiveSessions = []; // live terminal CLI sessions (not yet attached)
var searchQuery = "";
...
function renderSessionItem(s) {
  ...
  if (s.liveMirror) {
    textHtml += '<span class="session-live-badge" title="Mirroring a terminal session live">LIVE</span>';
  }
  textSpan.innerHTML = textHtml;
  el.appendChild(textSpan);
...
export function renderSessionList(sessions) {
  ...
  // Live terminal sessions (running on the host, not yet attached in Clay)
  var liveSection = renderLiveSessionsSection();
  if (liveSection) getSessionListEl().appendChild(liveSection);
...
// --- Live terminal sessions (host CLI sessions running right now) ---

function renderLiveSessionsSection() {
  if (!cachedLiveSessions || cachedLiveSessions.length === 0) return null;

  var section = document.createElement("div");
  section.className = "session-live-section";

  var header = document.createElement("div");
  header.className = "session-group-header session-live-header";
  header.innerHTML = '<span class="session-live-dot"></span>Live terminal';
  section.appendChild(header);

  for (var i = 0; i < cachedLiveSessions.length; i++) {
    var s = cachedLiveSessions[i];
    var el = document.createElement("div");
    el.className = "session-item session-live-item";

    var textSpan = document.createElement("span");
    textSpan.className = "session-item-text";
    var html = '<span class="session-live-badge">' + (s.kind === "bg" ? "BG" : "LIVE") + '</span>';
    html += escapeHtml((s.firstPrompt || "Terminal session").substring(0, 60));
    textSpan.innerHTML = html;
    el.appendChild(textSpan);

    if (s.gitBranch) {
      var branchSpan = document.createElement("span");
      branchSpan.className = "session-live-branch";
    ...
    (function (sessionId) {
      el.addEventListener("click", function () {
        if (getWs() && store.get('connected')) {
          // resume_session attaches a live mirror server-side when the
          // session is still active in the terminal
...
export function updateLiveSessions(sessions) {
...
export function populateCliSessionList(sessions) {
  ...
  for (var i = 0; i < sessions.length; i++) {
    ...
    title.appendChild(document.createTextNode(s.firstPrompt || "Untitled session"));
