...
import { renderSessionList, updateSessionPresence, populateCliSessionList, handleSearchResults, updateSessionBadge, updateLiveSessions } from './sidebar-sessions.js';
    ...
    switch (msg.type) {
      ...
      case "model_info": {
        ...
        if (Object.prototype.hasOwnProperty.call(msg, "model")) {
          var _vendorLocked = store.get('vendorSelectionLocked');
          var _existingModel = store.get('currentModel');
          // When vendor is locked, only keep existing model if it belongs to
          // this vendor's model list. Otherwise update (e.g. switching from
          // Claude session to Codex session — the lock shouldn't preserve
          // the old vendor's model name).
          if (_vendorLocked && _existingModel && msg.models && msg.models.length > 0) {
            var _modelBelongs = false;
            for (var _mbi = 0; _mbi < msg.models.length; _mbi++) {
              var _mbVal = msg.models[_mbi];
              if (typeof _mbVal === "object") _mbVal = _mbVal.value || _mbVal.id || "";
              if (_mbVal === _existingModel) { _modelBelongs = true; break; }
            }
            if (!_modelBelongs) {
              _miUpdate.currentModel = _modelVal || "";
            }
          } else if (!_vendorLocked || !_existingModel) {
            _miUpdate.currentModel = _modelVal || "";
          }
        } else {
      ...
      case "live_cli_sessions":
        updateLiveSessions(msg.sessions || []);
        break;

      case "live_mirror_state":
        // Live mirror attached/detached for a session; refresh via session_list broadcast.
        // Nothing to do beyond store sync for the active session banner.
        if (msg.id === store.get('activeSessionId')) {
          store.set({ liveMirrorActive: !!msg.active });
        }
        break;
      ...
      case "session_switched":
        ...
        store.set({ activeSessionId: msg.id, cliSessionId: msg.cliSessionId || null, vendorCapabilities: msg.capabilities || {}, sessionIsProcessing: !!msg.isProcessing, liveMirrorActive: !!msg.liveMirror });
        if (msg.vendor) {
