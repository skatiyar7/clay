...
function attachUserMessage(ctx) {
  ...
  function handleUserMessage(ws, msg) {
    ...
    // Takeover: sending a message from the browser ends live-mirror mode.
    // Clay resumes the CLI session itself from here on. But while the
    // terminal's CLI process still owns the session, resuming would conflict
    // ("Another Claude Code process is already running") — block and explain.
    if (session.liveMirror && liveSessions) {
      var liveInfo = liveSessions.getLiveInfo(session.cliSessionId);
      if (liveInfo) {
        sm.sendAndRecord(session, {
          type: "info",
          text: "This session is still open in a terminal (" + (liveInfo.name || "claude") + ", PID " + liveInfo.pid + "). Close it there (ctrl+c twice or /exit) to take over from the browser — mirroring continues in the meantime.",
        });
        return true;
      }
      liveSessions.detachTailForSession(session);
    }
