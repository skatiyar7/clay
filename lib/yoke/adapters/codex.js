...
function humanizeCodexActiveFlag(flag) {
  if (!flag) return "";
  if (flag === "waitingOnApproval") return "waiting on approval";
  if (flag === "waitingOnMcp") return "waiting on MCP";
  if (flag === "waitingOnMcpServer") return "waiting on MCP server startup";
  if (flag === "waitingOnToolCall") return "waiting on tool call";
  return String(flag).replace(/([a-z])([A-Z])/g, "$1 $2").toLowerCase();
}

function formatThreadStatusDetail(status) {
  if (!status || !status.type) return "";
  if (status.type === "active") {
    var flags = Array.isArray(status.activeFlags) ? status.activeFlags.map(humanizeCodexActiveFlag).filter(Boolean) : [];
    if (flags.length > 0) return "Codex is active: " + flags.join(", ") + ".";
    return "Codex is working...";
  }
  if (status.type === "idle") return "Codex is idle.";
  if (status.type === "notLoaded") return "Codex thread unloaded.";
  if (status.type === "systemError") return "Codex reported a system error.";
  return "Codex status: " + status.type;
}

function formatMcpStartupStatusDetail(name, status, error) {
  var label = name || "MCP server";
  if (status === "starting") return "Starting MCP server " + label + "...";
  if (status === "ready") return "MCP server " + label + " is ready.";
  if (status === "failed") {
    if (error) return "MCP server " + label + " failed to start: " + error;
    return "MCP server " + label + " failed to start.";
  }
  return "MCP server " + label + " status: " + (status || "unknown");
}
...
var FALLBACK_CODEX_MODELS = [
  { value: "gpt-5.6-terra", displayName: "gpt-5.6-terra", supportsEffort: true, supportedEffortLevels: ["minimal", "low", "medium", "high"] },
  { value: "gpt-5.6-sol", displayName: "gpt-5.6-sol", supportsEffort: true, supportedEffortLevels: ["minimal", "low", "medium", "high"] },
  { value: "gpt-5.6-luna", displayName: "gpt-5.6-luna", supportsEffort: true, supportedEffortLevels: ["minimal", "low", "medium", "high"] },
  { value: "gpt-5.5", displayName: "gpt-5.5", supportsEffort: true, supportedEffortLevels: ["minimal", "low", "medium", "high"] },
  { value: "gpt-5.4", displayName: "gpt-5.4", supportsEffort: true, supportedEffortLevels: ["minimal", "low", "medium", "high"] },
  { value: "gpt-5.4-mini", displayName: "gpt-5.4-mini", supportsEffort: true, supportedEffortLevels: ["minimal", "low", "medium", "high"] },
  { value: "gpt-5.3-codex", displayName: "gpt-5.3-codex", supportsEffort: true, supportedEffortLevels: ["minimal", "low", "medium", "high"] },
  { value: "gpt-5.3-codex-spark", displayName: "gpt-5.3-codex-spark", supportsEffort: true, supportedEffortLevels: ["minimal", "low", "medium", "high"] },
  { value: "gpt-5.2", displayName: "gpt-5.2", supportsEffort: true, supportedEffortLevels: ["minimal", "low", "medium", "high"] },
];
...
function flattenEvent(notification, state) {
  ...
  if (method === "warning") {
    if (params.message) {
      events.push({
        yokeType: "sdk_notification",
        text: "Codex warning: " + params.message,
      });
    }
    return events;
  }
  ...
  if (method === "mcpServer/startupStatus/updated") {
    var startupDetail = formatMcpStartupStatusDetail(params.name, params.status, params.error);
    if (startupDetail) {
      events.push({
        yokeType: "status_detail",
        status: "processing",
  ...
  if (method === "thread/status/changed") {
    var threadStatus = params.status || null;
    var threadDetail = formatThreadStatusDetail(threadStatus);
    if (threadStatus && threadStatus.type === "systemError") {
      events.push({
        yokeType: "error",
        text: threadDetail,
      });
    ...
    if (threadStatus && threadStatus.type === "active" && threadDetail) {
      events.push({
        yokeType: "status_detail",
        status: "processing",
        detail: threadDetail,
